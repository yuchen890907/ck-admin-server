...
const reducer = (state, action) => {
	switch (action.table) {
		...
		case "customization":
			if (typeof action.actions === "function")
				return { ...state, customization: action.actions(state.customization) };
			else return { ...state, customization: action.actions };
		case "customclasses":
			if (typeof action.actions === "function")
				return { ...state, customclasses: action.actions(state.customclasses) };
			else return { ...state, customclasses: action.actions };
		case "productcustom":
			if (typeof action.actions === "function")
				return { ...state, productcustom: action.actions(state.productcustom) };
			else return { ...state, productcustom: action.actions };
		case "sales":
...
export const ContextProvider = ({ children }) => {
	let loginInit = { isLogin: false, token: null, user: null };
	let emptyLoginState = { isLogin: false, token: null, user: null };
	const o = JSON.parse(localStorage.getItem("login")); // o = object
	if (o && o.success) {
		fetchService.token = o.token;
		loginInit = { isLogin: true, token: o.token, user: o.user };
	}
	...
	useEffect(() => {
		if (socket) {
			socket.emit("connected", { system: "admin" });
